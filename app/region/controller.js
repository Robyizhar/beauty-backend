...
async function getProvinsi(req, res, next){
    const data_provinsi = path.resolve(__dirname, './data/provinces.csv');
    try{
        const data = await csv().fromFile(data_provinsi);
...
async function getKabupaten(req, res, next){
    const data_kabupaten = path.resolve(__dirname, './data/regencies.csv');
    try{
        let {kode_induk} = req.query;
        const data = await csv().fromFile(data_kabupaten);
